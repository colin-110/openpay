...
The detail behind [the README](../README.md): every endpoint, what each mechanism does, and the
reasoning for the ones where the reasoning is the interesting part.

Split out because a README that answers "what is this and should I care" in two minutes is worth
more than one that answers every question and is therefore read by nobody. Nothing here was cut —
it moved.
...
The one-command form is in [the README](../README.md). These are the alternatives: running
services from Maven against Dockerised infrastructure, running them individually, local TLS,
and watching failover happen by hand.
